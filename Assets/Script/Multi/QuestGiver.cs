...
{
    //La Qûete à donner :
    public List<Quest> LesQuêtes;

    public Quest LaQûete;
    ...
    public override void Interract(Joueur player)
    {
        ...
        if (afficheounon)
        {
            ...
            player.CLaHess.QuestGiver = this;
            LaQûete.Avancement = 0;
            Cursor.visible = true;
            Cursor.lockState = CursorLockMode.None;
            player.Cam.GetComponent<ThirdPersonCameraControl>().IsPaused = true;
        }
        else
        {
    ...
    public void Resume() //Remise en marche du jeu
    {
        ...
        player.Cam.GetComponent<ThirdPersonCameraControl>().IsPaused = false;
    ...
    public void Update()
    {
        ...
        foreach (var quête in QuestName)
        {
            foreach (var quest in Unjoueur.GetComponent<Joueur>().Questnotdoneyet)
            {
                if (quest.Titre == quête)
                {
        ...
        {
            Condition = false;
            foreach (var quest in Unjoueur.GetComponent<Joueur>().listdesqêtes)
            {
                if (i < nbQuest && quest.Titre == LesQuêtes[i].Titre)
                {
                    i++;
